...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
	...
	const pusblishedBootcamp = await Bootcamp.findOne({ user: req.user.id });

	// If user is not admin they can only add one bootcamp
	if (pusblishedBootcamp && req.user.role !== 'admin') {
		return next(
			new ErrorResponse(
				`The user with ID ${req.user.id} has already pusblished a bootcamp`,
				400
			)
		);
	}
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
	...
	if (file.size > process.env.MAX_FILE_SIZE) {
		return next(
			new ErrorResponse(
				`Please upload an image less ${process.env.MAX_FILE_SIZE}`,
				400
			)
		);
	}
